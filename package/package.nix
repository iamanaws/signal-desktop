...
let
  nodejs = nodejs_24;
  pnpm = pnpm_11;
  # pnpm 11 on Darwin can get killed while materializing sticker-creator deps.
  stickerCreatorPnpm = pnpm_10_29_2;
  electron = electron_42;

  libsignal-node = callPackage ./libsignal-node.nix { inherit nodejs; };
  signal-sqlcipher = callPackage ./signal-sqlcipher.nix { inherit nodejs; };

  webrtc = callPackage ./webrtc.nix { };
  ringrtc = callPackage ./ringrtc.nix { inherit webrtc; };

  version = (builtins.fromJSON (builtins.readFile (localSrc + "/package.json"))).version;

  apple-emoji = localSrc + "/fonts/emoji.woff2";

  sticker-creator = stdenv.mkDerivation (finalAttrs: {
    pname = "signal-desktop-sticker-creator";
    inherit version;
    src = localSrc + "/sticker-creator";

    pnpmDeps = fetchPnpmDeps {
      inherit (finalAttrs) pname src version;
      pnpm = stickerCreatorPnpm;
      fetcherVersion = 4;
      hash = "sha256-FqW8AGFJacCxhGvEi2yF3zyjjJMalyFkwlKnBboHURk=";
    };
...
stdenv.mkDerivation (finalAttrs: {
  ...
  + lib.optionalString stdenv.hostPlatform.isDarwin ''
    # electron-builder's .icon asset path requires Xcode 26's actool.
    # Use a generated PNG source icon instead for local Nix builds on macOS.
    mkdir -p build
    magick -background none images/signal-logo.svg -resize 1024x1024 build/icon.png
    substituteInPlace package.json \
      --replace-fail '"icon": "build/icons/mac/AppIcon.icon",' '"icon": "build/icon.png",'
    substituteInPlace package.json \
      --replace-fail '"icon": "build/dmg/icon.icns",' '"icon": null,'
  '';
  ...
  pnpmDeps = fetchPnpmDeps {
    ...
    inherit pnpm;
    fetcherVersion = 4;
    hash = "sha256-kxtMGMwH6pEUDnakEga0KSFRxfZIxCNys/7OXqpgcO0=";
  };
